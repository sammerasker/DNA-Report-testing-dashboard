...
class QualityMetrics {
  ...
  _countUserTypeMentions(report, assessmentData) {
    let userType = assessmentData?.profile?.userType || '';
    
    // Handle userType being an array (new format) or string (old format)
    if (Array.isArray(userType)) {
      // Join array elements with pipe for regex OR matching
      userType = userType.map(type => type.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|');
    } else if (typeof userType === 'string' && userType) {
      // Escape regex special characters for string format
      userType = userType.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    } else {
      // Invalid or empty userType
      return {
        count: 0,
        minimum: 6,
        pass: false
