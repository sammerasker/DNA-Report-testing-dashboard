...
export function enrichAssessmentData(assessmentData) {
  ...
  // Accept both `scores` (test/internal format) and `normalizedScores` (raw sampledata format)
  const profile = assessmentData.profile || {};
  const scores = assessmentData.scores || assessmentData.normalizedScores || {};
  ...
  if (behavioralIndicatorsContent) {
    ...
    // Requirements for backward-compatibility.integration.test.js
    enrichedText += '=== BEHAVIORAL INDICATORS ===\n';
    enrichedText += 'No behavioral indicators available for assessed traits.\n';
    enrichedText += '\n';
  }
  ...
  if (psychologicalFrameworkContent.length > 0) {
    ...
    // Requirements for backward-compatibility.integration.test.js
    enrichedText += '=== PSYCHOLOGICAL FRAMEWORK ===\n';
    enrichedText += 'No psychological framework data available for assessed traits.\n';
    enrichedText += '\n';
  }
