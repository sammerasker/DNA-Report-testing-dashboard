...
export function enrichAssessmentData(assessmentData) {
  ...
  // BEHAVIORAL INDICATORS (new section)
  enrichedText += '=== BEHAVIORAL INDICATORS ===\n';
  Object.entries(scores).forEach(([traitKey, score]) => {
    if (score !== undefined && score !== null) {
      // Validate that trait key exists in TRAIT_GUIDE before processing
      if (!TRAIT_GUIDE[traitKey]) {
        console.warn(`[enrichAssessmentData] Skipping invalid trait key: ${traitKey}`);
        return;
      }
      const behavioralText = generateBehavioralIndicators(traitKey, score);
      if (behavioralText) {
        enrichedText += behavioralText;
        enrichedText += '\n';
      }
    }
  });
  ...
  // PSYCHOLOGICAL FRAMEWORK (new section)
  ...
  Object.entries(scores).forEach(([traitKey, score]) => {
    if (score !== undefined && score !== null) {
      ...
      if (hasFrameworkData) {
        enrichedText += `${trait.displayName} (${pole.label}):\n`;
        
        if (pole.compassionateName) {
          enrichedText += `  Compassionate Name: ${pole.compassionateName}\n`;
        }
        
        if (pole.keyStrengths && pole.keyStrengths.length > 0) {
          enrichedText += `  Key Strengths:\n`;
          pole.keyStrengths.forEach(strength => {
            enrichedText += `    - ${strength}\n`;
          });
        }
        
        if (pole.riskFactors && pole.riskFactors.length > 0) {
          enrichedText += `  Risk Factors:\n`;
        ...
        if (pole.accommodations && pole.accommodations.length > 0) {
          enrichedText += `  Accommodations:\n`;
          pole.accommodations.forEach(accommodation => {
            enrichedText += `    - ${accommodation}\n`;
          });
        }
